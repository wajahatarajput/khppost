...
const ComposePostComponent = () => {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const currentPost = {
      ...
      createdAt:  post?.id ? post?.createdOn : new Date().toLocaleString(),
      reactions: [],
      comments: [],
      ...
      <div className={`containerMain`}>
        <div className="forms">
          <div className={`form login`}>
            <span className="title">{post?._id ? 'Edit Post' : 'Compose Post'} </span>
            <form onSubmit={handleSubmit}>
              ...
              <div className="input-field">
                <input
                  ...
                  required
                  defaultValue={post?._id && post?.tags[0]}
                />
                <i className="uil uil-lock icon"></i>
              </div>
              <div className="input-field button">
                <input type="submit" value={post?._id ? 'Update' : 'Create'} />
              </div>
            </form>
          </div>
        </div>
